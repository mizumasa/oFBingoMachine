...
public:
    ...
    void update(ofEventArgs &e)
    {
        for (size_t i = mStrIndex; i < min(mStrIndex + mRange, mCurrentStr.length()); ++i) {
            mCurrentStr[i] = static_cast<char>(floor(ofRandom(97, 123)));
            //mCurrentStr[i] = static_cast<char>(floor(ofRandom(65, 91))+floor(ofRandom(0, 1.999))*32);
    ...
    const double getIntMillis()
    {
        return mIntervalMillis;
    }
    const double getLastMillis()
    {
        return mLastStoppedTimeMillis;
    }
